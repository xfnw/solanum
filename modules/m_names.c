...
names_global(struct Client *source_p)
{
	...
	sendto_one_multiline_init(source_p, " ", form_str(RPL_NAMREPLY),
				    me.name, source_p->name, "*", "*");
	...
	RB_DLINK_FOREACH(ptr, global_client_list.head)
	{
		...
		{
			sendto_one_multiline_item(source_p, "%s!%s@%s", target_p->name, target_p->username, target_p->host);
		}
		else
		{
			sendto_one_multiline_item(source_p, "%s", target_p->name);
		}
	}
