...
channel_member_names(struct Channel *chptr, struct Client *client_p, int show_eon)
{
	...
	if(ShowChannel(client_p, chptr))
	{
		...
		sendto_one_multiline_init(client_p, " ", form_str(RPL_NAMREPLY),
		...
		RB_DLINK_FOREACH(ptr, chptr->members.head)
		{
			...
			{
				sendto_one_multiline_item(client_p, "%s%s!%s@%s", find_channel_status(msptr, stack),
						  target_p->name, target_p->username, target_p->host);
			}
			else
			{
				sendto_one_multiline_item(client_p, "%s%s", find_channel_status(msptr, stack),
						  target_p->name);
			}
		}
